...
void fft_radix2_openmp(const std::vector<std::complex<float>>& input,
    ...
    #pragma omp parallel
    {
        ...
        for (int i = 0; i < N; i++) {
            ...
            if (i < j) {
                std::complex<float> temp = output[i];
                output[i] = output[j];
                output[j] = temp;
            }
        }
        ...
        for (int s = 1; s <= log2_N; s++) {
            ...
            #pragma omp for schedule(static)
            for (int butterfly_idx = 0; butterfly_idx < N / 2; butterfly_idx++) {
                const int group_idx = butterfly_idx / m_half;
                const int j = butterfly_idx % m_half;
